...
用法：
  ...
  codereview --pr https://github.com/owner/repo/pull/123  # 审查GitHub PR
  codereview --pr owner/repo/123                          # 简短格式
"""
...
def main() -> None:
    parser = ArgumentParser(
        description="自动化代码Review Bot - 分析git diff或GitHub PR并给出Review意见",
        epilog="示例:\n  codereview              # 审查未提交的改动\n  codereview --staged     # 审查已暂存的改动\n  codereview --commit HEAD~1  # 审查与上一次提交的差异\n  codereview --pr https://github.com/owner/repo/pull/123  # 审查PR",
    )
    ...
    parser.add_argument(
        ...
        default="",
        metavar="PR_URL",
        help="GitHub PR的URL（如 https://github.com/owner/repo/pull/123）或简短格式（owner/repo/123）",
    )
    ...
    if param_count > 1:
        print("❌ 错误：--staged、--commit 和 --pr 不能同时使用。请选择其中一个。")
        sys.exit(1)
    ...
    if args.pr:
        # GitHub PR Review
        user_input = f"请对这个GitHub PR ({args.pr}) 的代码变动进行Review，分析安全问题、性能问题和代码规范。"
    elif args.staged:
        # 暂存区的改动
        user_input = "请对我的代码变动 (git diff --staged，已暂存的改动) 进行Review，分析安全问题、性能问题和代码规范。"
    elif args.commit:
        # 指定提交范围
        user_input = f"请对我的代码变动 (git diff {args.commit}) 进行Review，分析安全问题、性能问题和代码规范。"
    else:
        # 默认：未提交的改动
